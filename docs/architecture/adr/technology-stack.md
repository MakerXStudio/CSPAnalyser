...
description: Why we chose Playwright, better-sqlite3, http-mitm-proxy, and MCP SDK
---
...
|------|-----------|--------|------------|
| http-mitm-proxy unmaintained (last published 2023) | Medium | Medium | MITM proxy is only used for remote HTTPS sites; local mode uses Playwright route interception. If the library fails on newer TLS, fork or replace with `@bjowes/http-mitm-proxy` (active fork) |
